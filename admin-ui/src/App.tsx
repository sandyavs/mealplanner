...
import { BulkUpload } from "./People/BulkUpload";
...
useEffect(() => {
	...
	.then((resolvedValue) => {
		setDataProvider(resolvedValue)
		//checking logs
		resolvedValue.getList("people", {
			pagination: { page: 1, perPage: 50 },
			sort: { field: "id", order: "ASC" },
			filter: {},
		  }).then((res) => {
			console.log("People records:", res.data);
		  }).catch((err) => {
	...
	<main>
		...
		>
			...
			<>
				...
				/>
				<Route path="people/bulk-upload" element={<BulkUpload />} />
				</CustomRoutes>
			</>
			)}
		</Admin>
